    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $("input[name='cancel']").click(function () {
                window.location.href="EntryManage";
            })
        });
    </script>
</head>
...
<div class="formbody" >
    <div class="formtitle"><span>入职信息</span></div>
    <form action="doUpdateEntry" method="post" name="form1">
    <ul class="forminfo">
        <li><label>学员班级</label>
            <select name="classNo" id="classno" style="line-height: 40px;margin-top: 10px;width: 80px;border: 1px solid #CCCCCC">
                <c:forEach items="${classList}" var="temp">
                    <c:choose>
                        <c:when test="${temp.classNo==entryM.classNo}">
        ...
        <li><label>学员姓名</label>
            <select name="studentNo" id="studentno" style="line-height: 40px;margin-top: 10px;width: 80px;border: 1px solid #CCCCCC">
                <c:forEach items="${studentList}" var="temp">
                    <c:choose>
                        <c:when test="${temp.traid==entryM.studentNo}">
        ...
        <li><label>就职时间</label><input name="eTime" type="text" class="dfinput" value="${entryM.eTime}"/></li>
        <li><label>入职企业</label>
            <select name="eeNo" id="enterpriseno" style="line-height: 40px;margin-top: 10px;width: 80px;border: 1px solid #CCCCCC">
                <c:forEach items="${enterpriseList}" var="temp">
                    <c:choose>
                        <c:when test="${temp.id==entryM.eeNo}">
        ...
        <li><label>就职待遇</label><input name="sal" type="text" class="dfinput" value="${entryM.sal}"></li>
        <li><label>就职反馈</label><input name="back" type="text" class="dfinput" value="${entryM.back}"/></li>
        <li>
        ...
        <input name="id" type="hidden" value="${entryM.id}">
    </form>
</div>
<div class="tip">
